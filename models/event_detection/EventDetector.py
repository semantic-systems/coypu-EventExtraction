...
class EventDetector(BaseComponent):
    def __init__(self, path_to_pretrained_model: str):
        super().__init__()
        self.model = torch.load(path_to_pretrained_model)
    ...
    def forward(self, tweet: str) -> EventDetectorOutput:
        ...
        prediction = output.prediction_logits.argmax(1)
        event_type = str(prediction)  # TODO: get label index map into model class
